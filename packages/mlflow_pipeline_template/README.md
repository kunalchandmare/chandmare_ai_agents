...
### Shared helpers (`shared/helpers.py`)
...
- The templates automatically bootstrap project-root import resolution so `shared.helpers` works when steps/components are launched via `mlflow.run(...)`.
- Add any additional reusable/common functions in `shared/helpers.py` (or nearby modules under `shared/`) when multiple steps/components should share the same logic.
